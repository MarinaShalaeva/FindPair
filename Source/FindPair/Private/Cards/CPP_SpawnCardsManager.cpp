...
{
	if (GameInstanceRef && CanShowGeneratedLevelHandle.IsValid())
	{
...
void UCPP_SpawnCardsManager::StartLevel(ELevelMode LevelMode)
{
	...
	CardsDelegatesRef->CardsSpawningEndedDelegate.ExecuteIfBound(NumberOfPairs, TimeToClose, CurrentLevelMode);
}
...
void UCPP_SpawnCardsManager::RandomizeCards()
{
	...
	TArray<TAssetSubclassOf<ACPP_ParentCard>> AllRandomClasses = AllCardsClasses;
...
void UCPP_SpawnCardsManager::FisherYatesShuffleAlgorithm(TArray<TAssetSubclassOf<ACPP_ParentCard>>& Arr, int32 ArrLen)
{
	const int32 Len = Arr.Num();
	for (int32 i = 0; i <= Len - 2; i++)
	{
		const int32 j = FMath::RandRange(i, Len - 1);
		const TAssetSubclassOf<ACPP_ParentCard> Tmp = Arr[i];
		Arr[i] = Arr[j];
		Arr[j] = Tmp;
		// Arr.Swap(i, j);
	}
