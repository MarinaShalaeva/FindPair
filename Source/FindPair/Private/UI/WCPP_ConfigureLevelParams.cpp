	...
	: UUserWidget(ObjectInitializer)
{
	...
	ChooseAllCardsStylesText = FText();
	DeleteAllCardsStylesText = FText();
}
...
void UWCPP_ConfigureLevelParams::NativeConstruct()
{
	...
	GI = GetWorld()->GetGameInstance<UCPP_GameInstance>();

	if (IsValid(GI) && GI->GetSavedLevelSettingsDelegate.IsBound())
	{
		int32 OutPairsNumber;
		ELevelMode OutLevelMode;
		TArray<ECardsStyles> OutCardsStyles;
		if (GI->GetSavedLevelSettingsDelegate.Execute(OutPairsNumber, OutLevelMode, OutCardsStyles))
		{
...
void UWCPP_ConfigureLevelParams::NativeDestruct()
{
	...
	if (IsValid(GI) && GI->SaveNewLevelSettingsDelegate.IsBound())
	{
		...
		GI->SaveNewLevelSettingsDelegate.Execute(PairsNumberToUse, LevelModeToUse, CardsStylesToUse);
	}
...
void UWCPP_ConfigureLevelParams::StartGameButtonOnClicked()
{
	...
	if (IsValid(GI))
	{
		if (GI->SaveNewLevelSettingsDelegate.IsBound())
		{
			GI->SaveNewLevelSettingsDelegate.Execute(PairsNumberToUse, LevelModeToUse, CardsStylesToUse);
		}
		GI->StartGameDelegate.Broadcast(PairsNumberToUse, LevelModeToUse, CardsStylesToUse);
	}
}
