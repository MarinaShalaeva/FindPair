...
UWCPP_MainMenu::UWCPP_MainMenu(const FObjectInitializer& ObjectInitializer) : UUserWidget(ObjectInitializer)
{
	...
	MainMenuInscription = FText();
	ChooseLevelInscription = FText();
	SettingsInscription = FText();
	PlayerResultsInscription = FText();
	GameInfoInscription = FText();
	...
	SaveWasDeletedSuccessfullyInscription = FText();
	CannotDeleteSaveInscription = FText();
...
{
	if (GetWorld()->GetTimerManager().TimerExists(ShowDeletingInfoTimer))
	{
