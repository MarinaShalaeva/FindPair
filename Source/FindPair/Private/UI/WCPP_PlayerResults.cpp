...
void UWCPP_PlayerResults::NativeConstruct()
{
	...
	if (WidgetTree && IsValid(GI) && GI->GetSavedPlayerResultsDelegate.IsBound())
	{
		TArray<struct FPlayerResult> Results;
		if (GI->GetSavedPlayerResultsDelegate.Execute(Results))
		{
			if (Results.Num() > 0)
			{
				...
				for (int32 i = Results.Num() - 1; i >= 0; i--)
				{
					...
					Args.Add(TEXT("SpentTimeInscriptionInscription"), SpentTimeInscription);
					...
					TB->SetText(FText::Format(LOCTEXT
						("ResultString",
						 ...
						 "{SpentTimeInscriptionInscription}: {TimeResult}\n"),
					                          Args));
