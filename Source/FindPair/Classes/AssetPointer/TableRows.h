...
/**
 * The table row for storing soft references
 * to cards' blueprints.
 */
...
struct FCardTableRow : public FTableRowBase
{
	...
	/**
	 * The sequential number of the card within
	 * the same table (of the same style).
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	int32 CardId;

	/**
	 * Soft reference to related card's blueprint
	 * class.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TAssetSubclassOf<ACPP_ParentCard> CardClass;
...
struct FSkyMaterialTableRow : public FTableRowBase
{
	...
	/**
	 * Rotation for the sky static mesh, which should
	 * be applied together with the material instance
	 * of SkyMaterialClass.
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	 ...
	 */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TAssetPtr<UMaterialInstanceConstant> SkyMaterialClass;

	FSkyMaterialTableRow() : AssetId(0),
	                         RelatedSkySphereRotation(FRotator(0.0f, 0.0f, 0.0f)),
